    ...
    students.forEach((student) => {
      ...
      semesterResults.forEach((learningInformation) => {
        if (learningInformation.semester === maxSemester) {
          ...
          else if (grade4 <= 2.495) data[1].count++;
          else if (grade4 <= 3.195) data[2].count++;
          else if (grade4 <= 3.595) data[3].count++;
          else data[4].count++;
        }
      });
    });
...
const getListSuggestedReward = async (req, res) => {
  try {
    ...
    students.forEach((student) => {
      (student.yearly_results || []).forEach((yr) => {
...
const getTopStudentsByLatestSemester = async (req, res) => {
  try {
    ...
    const allSemesters = [];
    students.forEach((student) => {
      (student.semester_results || []).forEach((li) => {
    ...
    students.forEach((student) => {
      const entry = (student.semester_results || []).find(
...
const getExcelCutRice = async (req, res) => {
  try {
    ...
    if (studentIdsQuery) {
      const studentIdArray = studentIdsQuery.split(",").map((id) => id.trim());
      students = students.filter((student) =>
        studentIdArray.includes(student.id)
      );
    }
    // Lọc theo đơn vị nếu có
    else if (unitQuery && unitQuery !== "all") {
...
const updateStudentCutRice = async (req, res) => {
  try {
    ...
    // Lấy lịch cắt cơm (mỗi sinh viên chỉ có 1 lịch)
    const currentCutRice = student.cut_rice;
    if (!currentCutRice) {
      return res.status(404).json({ message: "Không tìm thấy lịch cắt cơm" });
    }

    // Cập nhật lịch hiện có
    await currentCutRice.update({
      weekly: cutRiceData,
...
const generateAutoCutRiceForStudent = async (req, res) => {
  try {
    ...
    // Service sẽ tự động tạo hoặc cập nhật CutRice trong database
...
const getAllStudentsGrades = async (req, res) => {
  try {
    ...
    const allSemesterResultIds = students.flatMap((student) =>
      (student.semester_results || []).map((sr) => sr.id)
    );
    ...
    for (const student of students) {
      try {
        ...
        if (schoolYear) {
          ...
          // Tìm yearlyResult tương ứng trong student.yearly_results
          const existingYearlyResult = student.yearly_results?.find(
            (result) => result.schoolYear === schoolYear
          );
          ...
          if (student.yearly_results && student.yearly_results.length > 0) {
            student.yearly_results.forEach((yearlyResult) => {
...
const getWordTuitionFee = async (req, res) => {
  try {
    ...
    let allTuitionFees;
    try {
      allTuitionFees = await TuitionFee.findAll();
    } catch (dbError) {
      console.error("Lỗi kết nối database khi lấy dữ liệu học phí:", dbError);
      return res.status(500).json({
    ...
    // Lấy danh sách student IDs từ tuition fees
    const studentIds = [...new Set(allTuitionFees.map((f) => f.studentId))];

    // Lấy thông tin students với university và class
    let students;
    try {
    ...
    // Tạo mảng tuitionFees với đầy đủ thông tin
    let tuitionFees = allTuitionFees.map((tuitionFee) => {
      const student = studentMap.get(tuitionFee.studentId);
      return {
        id: tuitionFee.id,
        studentId: tuitionFee.studentId,
        fullName: student ? student.fullName : "",
        university: student ? student.university : "",
        unit: student ? student.unit : "",
    ...
    const totalBeforeFilter = tuitionFees.length;

    // Lọc theo học kỳ
    if (semesterQuery && semesterQuery !== "all") {
      const semesterArray = semesterQuery.split(",").map((s) => s.trim());
      tuitionFees = tuitionFees.filter((tuitionFee) => {
        return semesterArray.includes(tuitionFee.semester);
      });
    }
    ...
    // Lọc theo năm học
    if (schoolYearQuery && schoolYearQuery !== "all") {
      const schoolYearArray = schoolYearQuery.split(",").map((s) => s.trim());
      tuitionFees = tuitionFees.filter((tuitionFee) => {
        return schoolYearArray.includes(tuitionFee.schoolYear);
      });
    }
    ...
    // Lọc theo đơn vị
    if (unitQuery && unitQuery !== "all") {
      const unitArray = unitQuery.split(",").map((u) => u.trim());

      tuitionFees = tuitionFees.filter((tuitionFee) => {
        // So sánh chính xác trước, nếu không match thì mới dùng includes
        const isIncluded = unitArray.some((unitName) => {
          // So sánh chính xác với unit hoặc className
          if (
            tuitionFee.unit === unitName ||
            tuitionFee.className === unitName
          ) {
          ...
          // Nếu không match chính xác, kiểm tra contains (tránh false positive)
          if (
            (tuitionFee.unit && tuitionFee.unit.includes(unitName)) ||
            (tuitionFee.className && tuitionFee.className.includes(unitName))
          ) {
            return true;
          }
          return false;
    ...
    if (tuitionFees.length === 0) {
      ...
      return res.status(400).json({
        ...
        details: {
          ...
          totalAfterFilter: tuitionFees.length,
          filters: {
...
const getExcelPoliticalManagement = async (req, res) => {
  try {
    const { schoolYear, unit, studentIds, columns } = req.query;
    ...
    if (unit && unit !== "all") {
      queryConditions.push({ unit: unit });
    }

    // Thêm filter theo danh sách học viên nếu có
    if (studentIds) {
      const studentIdArray = studentIds.split(",").map((id) => id.trim());
      queryConditions.push({ id: { [Op.in]: studentIdArray } });
    }
    ...
    // Định nghĩa cấu trúc header dựa trên các cột được chọn
    const columnDefinitions = {
      ...
      foreignRelations: {
        header1: "Mối quan hệ có yếu tố nước ngoài",
        header2: "",
        width: 30,
        colspan: 1,
        merge: true,
      ...
      party: {
        header1: "Đảng",
        header2: ["Vào Đảng", "Chính thức"],
        width: 12,
        colspan: 2,
        merge: false,
      },
      partyRating: {
        header1: "Xếp loại Đảng viên",
        header2: ["Xuất sắc", "Tốt", "Hoàn thành", "Không hoàn thành"],
        width: 8,
        colspan: 4,
        merge: false,
      },
      rewards: {
        header1: "Khen thưởng",
        header2: ["CSTT", "CSTĐ", "BK BQP", "CSTĐ TQ"],
        width: 8,
        colspan: 4,
        merge: false,
      },
      trainingRating: {
        header1: "Xếp loại rèn luyện",
        header2: "",
        width: 15,
        colspan: 1,
        merge: true,
    ...
    // Xây dựng headers dựa trên cột được chọn
    const headers1 = [];
    const headers2 = [];
    const colWidths = [];
    ...
    selectedColumns.forEach((colKey) => {
      ...
      if (colDef.merge) {
        headers1.push(colDef.header1);
        headers2.push(colDef.header2);
        colWidths.push(colDef.width);
        mergeInfo.push({ type: "vertical", col: currentColIndex });
        currentColIndex++;
      } else {
        // Cột có nhiều sub-columns
        headers1.push(colDef.header1);
        for (let i = 1; i < colDef.colspan; i++) {
          headers1.push("");
        }
        colDef.header2.forEach((h) => headers2.push(h));
        for (let i = 0; i < colDef.colspan; i++) {
          colWidths.push(colDef.width);
        }
        mergeInfo.push({
          type: "horizontal",
          startCol: currentColIndex,
          endCol: currentColIndex + colDef.colspan - 1,
        });
        currentColIndex += colDef.colspan;
      }
    });
    ...
    let colOffset = 0;
    selectedColumns.forEach((colKey, idx) => {
      const colDef = columnDefinitions[colKey];
      if (!colDef) return;

      const startCol = colOffset + 1;
      const endCol = startCol + colDef.colspan - 1;
      const startColLetter = String.fromCharCode(64 + startCol);
      const endColLetter = String.fromCharCode(64 + endCol);

      if (colDef.merge) {
        // Merge dọc (row 3-4)
        worksheet.mergeCells(`${startColLetter}3:${startColLetter}4`);
      } else {
        // Merge ngang (row 3)
        if (colDef.colspan > 1) {
          worksheet.mergeCells(`${startColLetter}3:${endColLetter}3`);
        }
      }

      colOffset += colDef.colspan;
    });
    ...
    students.forEach((student, index) => {
      ...
      // Tạo data row dựa trên các cột được chọn
      const rowData = [];
      const leftAlignCols = []; // Track which columns need left alignment
      ...
      selectedColumns.forEach((colKey) => {
        ...
        switch (colKey) {
          case "stt":
            ...
            break;
          case "unit":
            rowData.push(student.unit || "Chưa có dữ liệu");
            break;
          case "personalInfo":
            leftAlignCols.push(rowData.length + 1);
            rowData.push(personalInfo);
            break;
          case "rank":
            rowData.push(student.rank || "Chưa có dữ liệu");
            break;
          case "family":
            leftAlignCols.push(rowData.length + 1);
            rowData.push(safeFamilyInfo);
            break;
          case "enlistment":
            rowData.push(enlistmentDate);
            break;
          case "ethnicity":
            rowData.push(student.ethnicity || "Chưa có dữ liệu");
            break;
          case "religion":
            rowData.push(student.religion || "Chưa có dữ liệu");
            break;
          case "foreignRelations":
            leftAlignCols.push(rowData.length + 1);
            rowData.push(safeForeignInfo);
            break;
          case "party":
            rowData.push(partyJoinDate);
            rowData.push(partyOfficialDate);
            break;
          case "partyRating":
            rowData.push(partyRating === "HTXSNV" ? "X" : ""); // Xuất sắc
            rowData.push(partyRating === "HTTNV" ? "X" : ""); // Tốt
            rowData.push(partyRating === "HTNV" ? "X" : ""); // Hoàn thành
            rowData.push(partyRating === "KHTNV" ? "X" : ""); // Không hoàn thành
            break;
          case "rewards":
            rowData.push(cstt);
            rowData.push(cstd);
      ...
      leftAlignCols.forEach((colNum) => {
        if (dataRow.getCell(colNum)) {
          dataRow.getCell(colNum).alignment = {
            horizontal: "left",
            vertical: "middle",
            wrapText: true,
          };
    ...
    // Xây dựng dòng tổng hợp theo các cột được chọn
    const summaryData = [];
    const leftAlignColumnsInSummary = [];
    let summaryColIndex = 1;
    let rewardsMergeStart = null;
    let rewardsMergeEnd = null;
    ...
    selectedColumns.forEach((colKey) => {
      ...
      switch (colKey) {
        case "stt":
          summaryData.push("TỔNG HỢP");
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          break;
        case "unit":
          summaryData.push(unitTotalText);
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          break;
        case "personalInfo":
          summaryData.push(`Tổng số học viên: ${totalStudents}`);
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          break;
        case "rank":
        ...
        case "enlistment":
          ...
          break;
        case "ethnicity":
          summaryData.push(ethnicityTotalText);
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          break;
        case "religion":
          ...
          break;
        case "foreignRelations":
          summaryData.push(foreignRelationsSummary);
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          break;
        case "party":
          summaryData.push(partyJoinDateSummary);
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          summaryData.push(partyOfficialDateSummary);
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          break;
        case "partyRating":
          summaryData.push(`HTXSNV: ${partyRatingXuatSac}`);
          summaryColIndex++;
          summaryData.push(`HTTNV: ${partyRatingTot}`);
          summaryColIndex++;
          summaryData.push(`HTNV: ${partyRatingHoanThanh}`);
          summaryColIndex++;
          summaryData.push(`KHTNV: ${partyRatingKhongHoanThanh}`);
          summaryColIndex++;
          break;
        case "rewards":
          rewardsMergeStart = summaryColIndex;
          summaryData.push(rewardSummary);
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          summaryData.push("");
          summaryColIndex++;
          ...
          summaryColIndex++;
          summaryData.push("");
          rewardsMergeEnd = summaryColIndex;
          summaryColIndex++;
          break;
        case "trainingRating":
          summaryData.push(trainingRatingSummary);
          leftAlignColumnsInSummary.push(summaryColIndex);
          summaryColIndex++;
          break;
        default:
    ...
    summaryRow.eachCell((cell, colNumber) => {
      ...
      cell.alignment = {
        horizontal: leftAlignColumnsInSummary.includes(colNumber)
          ? "left"
          : "center",
        vertical: "middle",
        wrapText: true,
    ...
    // Merge các cột khen thưởng nếu có
    if (rewardsMergeStart && rewardsMergeEnd) {
      const startCol = String.fromCharCode(64 + rewardsMergeStart);
      const endCol = String.fromCharCode(64 + rewardsMergeEnd);
      worksheet.mergeCells(
        `${startCol}${summaryRow.number}:${endCol}${summaryRow.number}`
      );
    }

    // Thiết lập độ rộng cột dựa trên cột được chọn
...
/**
 * Export Learning Results to Excel (Semester-based)
 */
const exportLearningResultsExcel = async (req, res) => {
  try {
    const {
      units,
      studentIds,
    ...
    if (units && units !== "all") {
      ...
      studentWhereClause.unit = { [Op.in]: unitArray };
    }
    if (studentIds) {
      const studentIdArray = studentIds.split(",").map((id) => id.trim());
      studentWhereClause.id = { [Op.in]: studentIdArray };
    }
    ...
    // Load subjects for all semester results
    const allSemesterResultIds = students.flatMap((student) =>
      (student.semester_results || []).map((sr) => sr.id)
    );
    ...
    for (const student of students) {
      ...
      // Filter by semester and schoolYear
      let filteredResults = [...semesterResults];
      if (semesters) {
        const semesterArray = semesters.split(",").map((s) => s.trim());
        filteredResults = filteredResults.filter((r) =>
          semesterArray.includes(r.semester)
        );
      }
      if (schoolYears) {
        const schoolYearArray = schoolYears.split(",").map((s) => s.trim());
        filteredResults = filteredResults.filter((r) =>
          schoolYearArray.includes(r.schoolYear)
        );
      }
      ...
      filteredResults.forEach((result) => {
        ...
        const normalizedSubjects = subjects.map((s) => {
          ...
          return {
            ...s,
            letterGrade: letter,
            gradePoint4: gradeHelper.letterToGrade4(letter),
            gradePoint10: gradeHelper.letterToGrade10(letter),
          };
        });
        ...
        const gpa4 = result.averageGrade4 || 0;
        const gpa10 = result.averageGrade10 || 0;
        const cpa4 = result.cumulativeGrade4 || 0;
        const cpa10 = result.cumulativeGrade10 || 0;

        // Filter by GPA range
        if (gpaMin && gpa4 < parseFloat(gpaMin)) return;
        if (gpaMax && gpa4 > parseFloat(gpaMax)) return;

        // Filter by CPA range
        if (cpaMin && cpa4 < parseFloat(cpaMin)) return;
        if (cpaMax && cpa4 > parseFloat(cpaMax)) return;
        ...
        allResults.push({
          ...
          cpa4: cpa4.toFixed(2),
          cpa10: cpa10.toFixed(2),
          totalCredits: result.totalCredits || 0,
          cumulativeCredits: result.cumulativeCredits || 0,
          failedSubjects,
          debtCredits,
          studentLevel: result.studentLevel || 1,
        });
    ...
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Kết quả học tập");
    ...
    worksheet.getCell("B2").font = { name: "Times New Roman", size: 13 };

    worksheet.mergeCells("C3:E3");
    worksheet.getCell("C3").value = "HỆ HỌC VIÊN 5";
    ...
    worksheet.getCell("I2").value = "CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM";
    ...
    worksheet.getCell("J3").value = "Độc lập - Tự do - Hạnh phúc";
    ...
    worksheet.mergeCells("K5:N5");
    const now = new Date();
    worksheet.getCell("K5").value = `Hà Nội, ngày ${now.getDate()} tháng ${
      now.getMonth() + 1
    } năm ${now.getFullYear()}`;
    ...
    worksheet.getCell("K5").font = { name: "Times New Roman", size: 13 };

    worksheet.mergeCells("D7:L7");
    worksheet.getCell("D7").value = "BẢNG KẾT QUẢ HỌC TẬP HỌC VIÊN HỆ 5";
    ...
    const summaryRow = worksheet.addRow([
      "TỔNG HỢP",
      "",
      `Tổng: ${allResults.length} kết quả`,
      "",
      "",
      "",
      "",
      `TB: ${(
        allResults.reduce((sum, r) => sum + parseFloat(r.gpa4), 0) /
        allResults.length
      ).toFixed(2)}`,
      "",
      `TB: ${(
        allResults.reduce((sum, r) => sum + parseFloat(r.cpa4), 0) /
        allResults.length
      ).toFixed(2)}`,
      "",
      "",
      "",
      allResults.reduce((sum, r) => sum + r.failedSubjects, 0),
      allResults.reduce((sum, r) => sum + r.debtCredits, 0),
    ]);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="ket-qua-hoc-tap-hoc-ky.xlsx"`
    );

    await workbook.xlsx.write(res);
    res.end();
  } catch (error) {
    console.error("Lỗi khi xuất Excel kết quả học tập:", error);
    res.status(500).json({
      success: false,
      message: "Lỗi server khi xuất file Excel",
    });
 ...
 */
const exportYearlyStatisticsExcel = async (req, res) => {
  try {
    const {
      units,
      studentIds,
    ...
    for (const student of students) {
      ...
      // Gắn subjects vào từng semester result
      semesterResults.forEach((sr) => {
        sr.subjects = subjectsBySemesterId[sr.id] || [];
      });
      ...
      semesterResults.forEach((sr) => {
        if (!semestersByYear[sr.schoolYear]) {
          semestersByYear[sr.schoolYear] = [];
        }
        semestersByYear[sr.schoolYear].push(sr);
      });

      // Filter by schoolYears if specified
      let targetYears = Object.keys(semestersByYear);
      if (schoolYears) {
        const schoolYearArray = schoolYears.split(",").map((s) => s.trim());
        targetYears = targetYears.filter((y) => schoolYearArray.includes(y));
      }

      // Process each year
      targetYears.forEach((year) => {
        const yearSemesters = semestersByYear[year] || [];
        if (yearSemesters.length === 0) return;
        ...
        yearSemesters.forEach((sr) => {
          const subjects = sr.subjects || [];
          subjects.forEach((s) => {
            const credits = s.credits || 0;
            totalCredits += credits;
            totalGradePoints4 += credits * (s.gradePoint4 || 0);
            totalGradePoints10 += credits * (s.gradePoint10 || 0);

            if (s.letterGrade === "F" || s.gradePoint4 === 0) {
              yearlyFailedSubjects++;
              yearlyDebtCredits += credits;
            }
          });
        });

        const yearlyGPA4 =
          totalCredits > 0 ? totalGradePoints4 / totalCredits : 0;
        const yearlyGPA10 =
          totalCredits > 0 ? totalGradePoints10 / totalCredits : 0;

        // Get cumulative from last semester
        const lastSemester = yearSemesters.sort((a, b) =>
          b.semester.localeCompare(a.semester)
        )[0];
        const cpa4 = lastSemester?.cumulativeGrade4 || 0;
        const cpa10 = lastSemester?.cumulativeGrade10 || 0;
        const cumulativeCredits = lastSemester?.cumulativeCredits || 0;

        // Get yearly result info
        const existingYearlyResult = yearlyResults.find(
          (yr) => yr.schoolYear === year
        );

        // Filter by GPA range
        if (gpaMin && yearlyGPA4 < parseFloat(gpaMin)) return;
        if (gpaMax && yearlyGPA4 > parseFloat(gpaMax)) return;
        ...
        allResults.push({
          ...
          yearlyGPA4: yearlyGPA4.toFixed(2),
          yearlyGPA10: yearlyGPA10.toFixed(2),
          cpa4: cpa4.toFixed(2),
          cpa10: cpa10.toFixed(2),
          totalCredits,
          cumulativeCredits,
          failedSubjects: yearlyFailedSubjects,
          debtCredits: yearlyDebtCredits,
          trainingRating: existingYearlyResult?.trainingRating || "",
          partyRating: existingYearlyResult?.partyRating || "",
          academicStatus: existingYearlyResult?.academicStatus || "",
        });
      });
    }
    ...
    allResults.sort((a, b) => {
      const valueA =
        sortBy === "cpa" ? parseFloat(a.cpa4) : parseFloat(a.yearlyGPA4);
      const valueB =
        sortBy === "cpa" ? parseFloat(b.cpa4) : parseFloat(b.yearlyGPA4);
      return sortOrder === "asc" ? valueA - valueB : valueB - valueA;
    });
    ...
    worksheet.getCell("J2").value = "CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM";
    ...
    worksheet.getCell("K3").value = "Độc lập - Tự do - Hạnh phúc";
    ...
    const now = new Date();
    worksheet.getCell("L5").value = `Hà Nội, ngày ${now.getDate()} tháng ${
      now.getMonth() + 1
    } năm ${now.getFullYear()}`;
    ...
    worksheet.getCell("L5").font = { name: "Times New Roman", size: 13 };

    worksheet.mergeCells("D7:M7");
    worksheet.getCell("D7").value =
      "BẢNG THỐNG KÊ KẾT QUẢ HỌC TẬP THEO NĂM HỌC";
    worksheet.getCell("D7").alignment = {
      vertical: "middle",
      horizontal: "center",
    };
    ...
    // Summary row
    const avgYearlyGPA =
      allResults.reduce((sum, r) => sum + parseFloat(r.yearlyGPA4), 0) /
      allResults.length;
    const avgCPA =
      allResults.reduce((sum, r) => sum + parseFloat(r.cpa4), 0) /
      allResults.length;
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="thong-ke-ket-qua-nam-hoc.xlsx"`
    );

    await workbook.xlsx.write(res);
    res.end();
  } catch (error) {
    console.error("Lỗi khi xuất Excel thống kê năm học:", error);
    res.status(500).json({
      success: false,
      message: "Lỗi server khi xuất file Excel",
    });
